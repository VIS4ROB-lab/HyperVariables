...
class SU2Metric final : public Metric<TScalar> {
 public:
  // Definitions.
  ...
  /// \param J_rhs Jacobian w.r.t. right element (optional).
  /// \return Distance between elements.
  auto evaluate(const Input& lhs, const Input& rhs, TScalar* J_lhs = nullptr, TScalar* J_rhs = nullptr) const -> Output { return Evaluate(lhs, rhs, J_lhs, J_rhs); }
};
